...
@section('view_site_url', route('guide.show', $guide->slug))
    ...
    <form id="guideForm" method="POST" action="{{ route('admin.guides.update', $guide->id) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="category">
                        ...
                        @foreach($platforms as $platform)
                            <option value="{{ $platform->name }}" {{ (isset($guide) && $guide->tags == $platform->name) ? 'selected' : '' }}>{{ $platform->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="form-card">
            ...
            <div class="form-group">
                ...
                <input type="text" name="meta_keywords" value="{{ $guide->meta_keywords }}" placeholder="keyword1, keyword2, ...">
            </div>
            <div class="form-group">
                <label>Robots Tag</label>
                <select name="meta_robots">
                    <option value="index, follow" {{ $guide->meta_robots == 'index, follow' ? 'selected' : '' }}>Index, Follow</option>
                    <option value="noindex, follow" {{ $guide->meta_robots == 'noindex, follow' ? 'selected' : '' }}>Noindex, Follow</option>
                    <option value="index, nofollow" {{ $guide->meta_robots == 'index, nofollow' ? 'selected' : '' }}>Index, Nofollow</option>
                    <option value="noindex, nofollow" {{ $guide->meta_robots == 'noindex, nofollow' ? 'selected' : '' }}>Noindex, Nofollow</option>
                </select>
            </div>
            <div class="form-group">
                <label>Custom Schema (JSON-LD)</label>
                <textarea name="schema" rows="5" placeholder='<script type="application/ld+json">&#10;{&#10;  "@context": "https://schema.org",&#10;  "@type": "Article",&#10;  "headline": "Example Title"&#10;}&#10;</script>'>{{ $guide->schema }}</textarea>
            </div>
        </div>
